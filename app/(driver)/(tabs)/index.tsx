...
import { acceptOrder, upsertDriverLocation } from '../../../services/driver.service';
...
function SwipeableCard({
    ...
    const flyOff = (toValue: number, callback: () => void) => {
        Animated.parallel([
            Animated.timing(translateX, { toValue, duration: 250, useNativeDriver: true }),
    ...
    const panResponder = useRef(
        PanResponder.create({
            ...
            onPanResponderRelease: (_, g) => {
                ...
                else if (g.dx > SWIPE_THRESHOLD) flyOff(SCREEN_WIDTH, onAccept);
                else snapBack();
            },
        })
    ).current;
